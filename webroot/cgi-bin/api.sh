...
# Handles HTTP API requests for overlay permission management
...
if [ "$ACTION" = "list" ]; then
    # Return JSON containing only apps requesting overlay permission and their live AppOps status
    echo "{"
    echo "  \"status\": \"success\","
    echo "  \"apps\": ["
    ...
    pm list packages -3 | cut -d: -f2 | sort | while read -r pkg; do
        ...
        if dumpsys package "$pkg" 2>/dev/null | grep -q "android.permission.SYSTEM_ALERT_WINDOW"; then
            ...
            # Check current live AppOps state
    ...
    # Check if it is a system default package (cannot toggle system packages)
    is_system=false
    for expkg in $EXCLUDED_PACKAGES; do
    ...
    if [ "$is_system" = false ]; then
        if grep -q "^$PACKAGE$" "$WHITELIST_FILE" 2>/dev/null; then
            # Remove from whitelist - block overlay
            grep -v "^$PACKAGE$" "$WHITELIST_FILE" > "${WHITELIST_FILE}.tmp"
            mv "${WHITELIST_FILE}.tmp" "$WHITELIST_FILE"
            status="removed"
            # Immediately block overlays/bubbles
            ...
            # Add to whitelist - allow overlay
